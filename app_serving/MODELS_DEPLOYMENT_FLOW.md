...
See [MODEL_STORAGE.md](MODEL_STORAGE.md) for the full directory layout and recovery
procedure.
...
`infra/on-prem/ansible/group_vars/all/images.yml`. Run after adding any new entry:
...
Model definitions live in `infra/model-registry/models.yml`. Each entry maps one model
...
- Upload model container images to Harbor (see `infra/on-prem/ansible/group_vars/all/images.yml`).
