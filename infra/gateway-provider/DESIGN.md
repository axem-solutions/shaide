...
### Azure — AGC (regions with Application Gateway for Containers)
...
- Requires `Microsoft.ServiceNetworking/trafficControllers` (not available in all regions)
...
### Azure — Istio (regions without AGC)

```
Client
    │
    ▼
Azure LB L4 (static Public IP from mc-aks-<stack> resource group)
    │
    ▼
shared-gateway-istio Service (LoadBalancer)  [gateway-system]
    │
...
- Self-hosted L7 proxy — Envoy runs as a pod inside the cluster
- Uses `gatewayClassName: istio`; `albName` is not set (no AGC resources created)
- A static Public IP is pre-allocated in the AKS node resource group (`mc-aks-<stack>`) by 03_cluster
  and bound to the Gateway via `spec.addresses` with `type: IPAddress`
- DNS uses an A record pointing to the static IP
- Health probe workaround: Azure LB defaults to HTTP `/` for health checks, but Istio returns 404.
  Gateway `spec.infrastructure.annotations` propagate the probe override to the generated Service,
  routing health checks to Istio's status port (15021 `/healthz/ready`)
- Works in any Azure region

### Istio vs AGC comparison

| | AGC | Istio |
|---|---|---|
| Managed by | Microsoft (outside cluster) | Self-hosted (pod in cluster) |
| GatewayClass | `azure-alb-external` | `istio` |
| IP / DNS | Stable hostname (CNAME) | Static Public IP (A record) |
| Region availability | Limited ([see list](https://learn.microsoft.com/en-us/azure/application-gateway/for-containers/overview#supported-regions)) | Any Azure region |
| Resource overhead | None in cluster | Envoy pod + istiod |
| Health probes | Handled by AGC | Requires annotation workaround |
| Config keys | `albName`, `gatewayClassName: azure-alb-external` | `gatewayClassName: istio` (no `albName`) |
...
Use **AGC** when the cluster is in a [supported region](https://learn.microsoft.com/en-us/azure/application-gateway/for-containers/overview#supported-regions)
— it offloads L7 routing to Azure, reduces in-cluster resource usage, and requires no health
probe workarounds.

Use **Istio** in regions where AGC is unavailable (`polandcentral` at time of writing). Istio is
already installed by gateway-provider for inference routing (per-model gateways), so the only
additional cost is the `shared-gateway-istio` Envoy pod and the Azure Standard LB with a static IP.
