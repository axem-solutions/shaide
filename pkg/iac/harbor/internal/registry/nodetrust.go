...
const nodeTrustNamespace = "node-registry-config"
...
func deployNodeTrustResources(
	...
	ns, err := corev1.NewNamespace(
		ctx,
		"node-registry-config-namespace",
	...
	harborHost := cfg.Network.RegistryHostname
	...
	script := fmt.Sprintf(
		nodeTrustScriptTemplate,
		certsDir,
		cfg.Network.StaticClusterIP,
	)

	labels := pulumi.StringMap{
		"app": pulumi.String(nodeTrustNamespace),
	}

	_, err = appsv1.NewDaemonSet(
		ctx,
