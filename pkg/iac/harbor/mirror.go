...
// deployImageMirror creates the harbor-image-mirror namespace, a one-shot
...
func deployImageMirror(
	...
	ns, err := corev1.NewNamespace(ctx, "harbor-image-mirror-namespace", &corev1.NamespaceArgs{
	...
	cm, err := corev1.NewConfigMap(ctx, mirrorImagesConfigMapName, &corev1.ConfigMapArgs{
		Metadata: &metav1.ObjectMetaArgs{
		...
		Data: pulumi.StringMap{
			...
			// credentials skip private images entirely. harbor:ghcrPinnedImages
			// isn't here: "pinned" mode doesn't use the CronJob/discover step
			// at all, see deployPinnedImagesJob.
			"min-versions.txt": pulumi.String(cfg.GhcrMinVersions),
		},
	}, providerOpt, pulumi.DependsOn([]pulumi.Resource{ns}))
	if err != nil {
		return fmt.Errorf("harbor-mirror-images configmap: %w", err)
	}

	secret, err := corev1.NewSecret(ctx, mirrorCredsSecretName, &corev1.SecretArgs{
		Metadata: &metav1.ObjectMetaArgs{
			Name:      pulumi.String(mirrorCredsSecretName),
			Namespace: pulumi.String(mirrorNamespace),
		},
		...
		StringData: pulumi.StringMap{
			"robot-user":     pulumi.String(harborRobotUser),
			"robot-password": robotPassword,
			"ghcr-user":      pulumi.String(cfg.GhcrUser),
			"ghcr-token":     cfg.GhcrToken,
		},
	}, providerOpt, pulumi.DependsOn([]pulumi.Resource{ns}))
	if err != nil {
		return fmt.Errorf("harbor-mirror-creds secret: %w", err)
	}

	harborHost := fmt.Sprintf("harbor.%s.svc.cluster.local", cfg.Namespace)

	if err := deployPublicImagesJob(ctx, k8sProvider, cfg, harborHost, ns, secret); err != nil {
		return fmt.Errorf("harbor-image-mirror-public job: %w", err)
	}

	if cfg.GhcrSyncMode == SyncModePinned {
		if err := deployPinnedImagesJob(ctx, k8sProvider, cfg, harborHost, ns, secret); err != nil {
			return fmt.Errorf("harbor-image-mirror-pinned job: %w", err)
		}
	} else {
		if err := deployPrivateImagesCronJob(ctx, k8sProvider, cfg, harborHost, ns, cm, secret); err != nil {
			return fmt.Errorf("harbor-image-mirror cronjob: %w", err)
		}
	}
...
// mirror.sh falls back to anonymous source access (see its own comments).
func deployPublicImagesJob(
	...
	_, err := batchv1.NewJob(ctx, mirrorPublicJobName, &batchv1.JobArgs{
		Metadata: &metav1.ObjectMetaArgs{
					...
					Containers: corev1.ContainerArray{
						&corev1.ContainerArgs{
							...
							Env: corev1.EnvVarArray{
								&corev1.EnvVarArgs{Name: pulumi.String("HARBOR_HOST"), Value: pulumi.String(harborHost)},
								&corev1.EnvVarArgs{Name: pulumi.String("MIRROR_LIST"), Value: pulumi.String(cfg.PublicImages)},
								secretEnvVar("HARBOR_ROBOT_USER", "robot-user", false),
								secretEnvVar("HARBOR_ROBOT_PASSWORD", "robot-password", false),
							},
							Command: pulumi.StringArray{pulumi.String("bash"), pulumi.String("-c")},
							Args:    pulumi.StringArray{pulumi.String(mirrorScript)},
						},
					},
				},
			},
		},
	}, providerOpt, pulumi.DependsOn([]pulumi.Resource{ns, secret}), pulumi.ReplaceOnChanges([]string{"spec"}))
	if err != nil {
		return err
	}
...
func deployPinnedImagesJob(
	...
	_, err := batchv1.NewJob(ctx, mirrorPinnedJobName, &batchv1.JobArgs{
		Metadata: &metav1.ObjectMetaArgs{
				...
				Spec: &corev1.PodSpecArgs{
					...
					Containers: corev1.ContainerArray{
						&corev1.ContainerArgs{
							...
							Env: corev1.EnvVarArray{
								&corev1.EnvVarArgs{Name: pulumi.String("HARBOR_HOST"), Value: pulumi.String(harborHost)},
								&corev1.EnvVarArgs{Name: pulumi.String("MIRROR_LIST"), Value: pulumi.String(cfg.GhcrPinnedImages)},
								&corev1.EnvVarArgs{Name: pulumi.String("MIRROR_REQUIRES_AUTH"), Value: pulumi.String("true")},
								secretEnvVar("HARBOR_ROBOT_USER", "robot-user", false),
								secretEnvVar("HARBOR_ROBOT_PASSWORD", "robot-password", false),
								secretEnvVar("GHCR_USER", "ghcr-user", true),
								secretEnvVar("GHCR_TOKEN", "ghcr-token", true),
							},
							Command: pulumi.StringArray{pulumi.String("bash"), pulumi.String("-c")},
							Args:    pulumi.StringArray{pulumi.String(mirrorScript)},
						},
...
func deployPrivateImagesCronJob(
	...
	staticAndDiscoveredMounts := corev1.VolumeMountArray{
		&corev1.VolumeMountArgs{Name: pulumi.String("discovered"), MountPath: pulumi.String("/images")},
		&corev1.VolumeMountArgs{Name: pulumi.String("static"), MountPath: pulumi.String("/images-static")},
	}

	_, err := batchv1.NewCronJob(ctx, mirrorCronJobName, &batchv1.CronJobArgs{
		Metadata: &metav1.ObjectMetaArgs{
						...
						Spec: &corev1.PodSpecArgs{
							...
							InitContainers: corev1.ContainerArray{
								&corev1.ContainerArgs{
									...
									Env: corev1.EnvVarArray{
										&corev1.EnvVarArgs{Name: pulumi.String("GHCR_ORG"), Value: pulumi.String(cfg.GhcrOrg)},
										&corev1.EnvVarArgs{Name: pulumi.String("GHCR_SYNC_MODE"), Value: pulumi.String(string(cfg.GhcrSyncMode))},
										secretEnvVar("GHCR_TOKEN", "ghcr-token", true),
									},
									Command:      pulumi.StringArray{pulumi.String("sh"), pulumi.String("-c")},
									Args:         pulumi.StringArray{pulumi.String(discoverScript)},
							...
							Containers: corev1.ContainerArray{
								&corev1.ContainerArgs{
									...
									Env: corev1.EnvVarArray{
										&corev1.EnvVarArgs{Name: pulumi.String("HARBOR_HOST"), Value: pulumi.String(harborHost)},
										&corev1.EnvVarArgs{Name: pulumi.String("MIRROR_LIST_FILE"), Value: pulumi.String("/images/private-images.txt")},
										&corev1.EnvVarArgs{Name: pulumi.String("MIRROR_REQUIRES_AUTH"), Value: pulumi.String("true")},
										secretEnvVar("HARBOR_ROBOT_USER", "robot-user", false),
										secretEnvVar("HARBOR_ROBOT_PASSWORD", "robot-password", false),
										secretEnvVar("GHCR_USER", "ghcr-user", true),
	...
	}, providerOpt, pulumi.DependsOn([]pulumi.Resource{ns, cm, secret}))
	if err != nil {
		return err
	}
